...
// normalizeTheme, DEFAULT_SETTINGS, SUPPORTED_LANGS).
//
...
function postToFrame(message) {
  el.frame.contentWindow?.postMessage(message, location.origin);
}
...
function closeTab() {
  Platform.runtime.closeOnboarding();
}
...
el.next.addEventListener("click", () => {
  // Leaving the tab-lock step: ask for host access now, while we still have the
  // click gesture chrome.permissions.request requires. Fired synchronously so
  // the gesture isn't lost; the result just grants/denies (default is lock-on).
  if (STEPS[stepIndex]?.group === "lock" && Platform.permissions) {
    Platform.permissions.ensureLockAccess();
  }
...
// language (it reads the platform store on init). The frame is hidden during the
