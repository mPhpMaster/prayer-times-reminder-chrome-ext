// overlay-tasbih.js — the dhikr (tasbih) reminder card (shared core UI).
// Platform-agnostic: no chrome.*, no Tauri, no Capacitor. Driven by
// window.__prayerTasbihActivate(config). Used by the extension content script,
// the Tauri dhikr window, and (later) mobile.
...
(function () {
  ...
  function activateTasbih(config) {
    // Always start clean: remove any card already on screen and cancel its
    // timer, so re-triggering (e.g. clicking Test dhikr again) shows a fresh
    // card instead of silently doing nothing.
    clearTasbih();

    const { display, text, label, dir = "ltr", lang = "en", position = "bottom-right", theme = "midnight-emerald", durationMs } = config;
    if (!display && !text) return;
    const dismissMs = Number(durationMs) > 0 ? Number(durationMs) : DISMISS_MS;
    ...
    shadow.innerHTML = `
      <style>
        ...
        @media (prefers-reduced-motion: reduce) {
          .tasbih-card, .tasbih-card.fade-out { animation: none; }
        }
  ...
  // persistent message listener is left behind in the page.
  window.__prayerTasbihActivate = activateTasbih;
  window.__prayerTasbihClear = clearTasbih;
})();
