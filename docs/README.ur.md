...
- 📿 **Periodic dhikr** — اختیاری فلوٹنگ ریمائنڈر جس میں کھلے ٹیبز پر 151 منفرد جملے ہوتے ہیں؛ چھپانے کے لیے دبائیں یا 10 سیکنڈ بعد خودکار طور پر ہٹ جائے گا۔
...
| Allow manual unlock | لاک اسکرین پر کہیں بھی ٹیپ کر کے (یا Esc دبا کر) اسے جلدی ختم کرنے دیتی ہے۔ |
| Test tab lock | موجودہ ٹیب پر لاک اوورلے کی پیش نظارہ (عام ویب سائٹس پر کام کرتا ہے، `chrome://` صفحات پر نہیں)۔ |
| Periodic dhikr | کھلے ٹیبز پر مقررہ یا بے ترتیب وقفے پر (1–120 منٹ) ایک رینڈم dhikr دکھاتی ہے۔ |
...
| `overlay-lock.js` | Injected overlay (shadow DOM) جو ٹائمر ختم ہونے تک یا صارف کے دستی ان لاک تک صفحے کی انٹر ایکشن روک دے۔ |
| `overlay-tasbih.js` | Injected floating dhikr کارڈ؛ دبانے پر یا 10 سیکنڈ بعد غائب۔ |
...
| `scripts/make_icons.py` | PNG آئیکنز دوبارہ بناتا ہے (صرف dev کے لیے؛ runtime میں ضروری نہیں)۔ |
...
- **Dhikr reminder:** اگر فعال ہو تو، `chrome.alarms` ٹائمر `tasbih-phrases.js` سے ایک رینڈم جملہ کھلے ٹیبز پر مقررہ وقفے یا min/max رینج کے اندر بے ترتیب وقفے پر دکھاتا ہے۔ کارڈ صفحہ بلاک نہیں کرتا؛ چھپانے کے لیے دبائیں یا 10 سیکنڈ انتظار کریں۔
...
<p align="center">
<sub>اور تمام مسلمان زندوں اور مردوں کی طرف سے۔</sub>
</p>